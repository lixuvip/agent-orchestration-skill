...
<p align="center">
  <strong>让 Codex 主线程变成协调者：分发角色线程、接收分支回调、跑 QA/审查门禁、做合并就绪检查。</strong>
</p>

<p align="center">
  <a href="README.md">English</a> ·
  <a href="#快速开始">快速开始</a> ·
  <a href="#v012-更新内容">更新内容</a> ·
  <a href="#演示流程">演示流程</a> ·
...
它尤其适合“另一个 Codex 线程/分支正在改东西，主线程需要可靠收口”的场景：回调主线程、请求状态、检查 QA/审查结果、判断分支是否可以合并或推送。
...
- [复制可用示例 Prompt](docs/examples.zh-CN.md)：[研究任务](examples/simple-research-task.md)、[编码加审查](examples/coding-review-workflow.md)、[分支回调](examples/branch-callback-controller-loop.md)、[产品规划](examples/multi-agent-product-planning.md)
- [查看 v0.1.2 更新说明](docs/releases/v0.1.2.md)
- [查看英文文档](README.md)
- [发布或 Fork 自己的版本](docs/publishing.zh-CN.md)

## v0.1.2 更新内容

`v0.1.2` 聚焦真实 Codex 分支和多线程工作流的主控效率。

| 模块 | 更新内容 | 价值 |
| --- | --- | --- |
| 编排确认 | 新增规则和模板，用于判断是否要先确认分支、线程、回调、自动化、合并或推送。 | 少问没必要的问题，但在高风险动作前会停下来确认。 |
| 主控循环 | 新增主线程循环：分发、回调、heartbeat、状态请求、合并就绪。 | 子线程可以独立工作，最终验收权仍在主线程。 |
| 分支交接 | 新增回调和合并就绪模板，包含分支/工作区、commit、测试、风险和下一步。 | 分支更容易被审查、验证、合并或交接。 |
| 长任务巡检 | heartbeat 可以在缺少状态或验证时主动发状态请求。 | 避免因为沉默或弱证据误判任务完成。 |
