...
如果目标项目允许写入，外部只读任务的质量也会按项目追加到 `.codex/agent-orchestration/agy-review-quality.jsonl`。审查默认记为 `task_type=review`，调研记为 `task_type=research`。首次在项目里使用时，流程会先把稳定的“Gemini 只能通过 agy 调用”的命令安全规则写入目标 `AGENTS.md`，后续 Codex 运行就不需要你反复提醒；质量日志只有看到 `LOG_WRITTEN` 才视为已写入。
...
当本机已经安装 `agy`，协调者可以在 Codex 实现后或接受分支交付前，运行一次有边界的外部模型审查。该流程使用 `references/AGY_GEMINI_REVIEW.md` 和 `references/templates/` 下的审查 prompt、质量评估模板和专属报告模板。

标准外部第二意见统一使用 `Gemini 3.5 Flash (High)`，包括普通第二意见和全项目基线盘点。全项目审查或用户要求对比时，流程会使用 Codex + Gemini 双轨审查：Codex reviewer 角色和 `agy` 独立审同一范围，最后由协调者对比共同命中、Gemini-only、Codex-only、被驳回 findings 和实际验证证据。这里的 Gemini 只能表示通过本机 `agy` 调用的 Gemini，不能替换成 standalone `gemini` CLI。流程会先用 `scripts/ensure_agy_review_agents_guidance.py` 在目标项目 `AGENTS.md` 写入稳定命令安全规则，而且这一步现在必须早于任何 `agy` 健康检查或模型探测。这个路径的探测命令只允许 `command -v agy` 和 `agy models`，不要再跑 `command -v gemini`、`gemini --version` 或 `gemini --help`。审查 prompt 模板里现在还加入了显式的反向护栏，用来压制 CLI/auth 叙述、伪造命令验证、范围膨胀和泛泛而谈的 filler。然后通过 `scripts/run_agy_print.py` 走普通只读 print + `--sandbox` 模式，保证 prompt 紧跟在 `--print` 后面，同时支持 `--add-dir <项目根目录>`，避免仓库审查时误读 Antigravity scratch，默认把 0 输出视为失败，并可用 `--expect-substring <标记>` 强制健康检查或结构化输出。如果有人误开或误探测 `gemini` CLI，或者从那个 CLI 返回 `403`，按 `WRONG_EXECUTION_SURFACE` 处理并改回 `agy` 重跑。外部 findings 必须被协调者逐条分类后才能采用，并以聊天中的专属审查报告展示；除非目标项目只读或隐私规则禁止写入，否则会运行 `scripts/append_agy_review_quality_log.py` 把质量记录追加到 `.codex/agent-orchestration/agy-review-quality.jsonl`。
...
标准调研模型也是 `Gemini 3.5 Flash (High)`，适合快速仓库盘点、方案扩展和实现路线比较。协调者仍然必须自己读仓库；如果问题依赖时间敏感的外部事实，仍由 Codex 去查一手来源并核验。这里的 Gemini 同样只能表示通过本机 `agy` 调用的 Gemini，不能替换成 standalone `gemini` CLI。可写仓库里的外部调研现在也要求先过 AGENTS 指引门，再做任何外部调研前置检查；探测命令仍然只允许 `command -v agy` 和 `agy models`。调研 prompt 模板里现在也加入了显式的反向护栏，用来压制 CLI/auth 叙述、范围膨胀、伪造验证和无锚点发散。如果有人误开或误探测 `gemini` CLI，或者从那个 CLI 返回 `403`，按 `WRONG_EXECUTION_SURFACE` 处理并改回 `agy` 重跑。Gemini 的作用是补一条独立研究流，而不是代替 Codex。最终结果会以专属调研报告展示共同观点、Gemini-only、Codex-only、驳回/推测性观点和建议下一步，并通过 `scripts/append_agy_review_quality_log.py` 以 `task_type=research` 写入同一个质量账本。
