...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model",
        help="Optional exact agy model name. Omit to use agy's configured default.",
    )
...
def build_command(args: argparse.Namespace, prompt: str) -> list[str]:
    ...
    if args.agy_bin != "agy" and os.environ.get("AGY_SECOND_OPINION_TESTING") != "1":
    ...
    command.extend(["--print-timeout", args.print_timeout, "--print", prompt])
    if args.model:
