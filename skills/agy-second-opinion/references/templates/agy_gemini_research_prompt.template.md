...
Use this template as the prompt for `scripts/run_agy_print.py`. Gemini means Gemini via agy only, never the standalone `gemini` CLI. Prefer a bounded prompt. If source context is required, attach only an allowlisted bundle produced by `scripts/build_agy_context_bundle.py`; attach the full repository only with explicit disclosure approval. Keep `--expect-substring 'AGY_RESEARCH_V1'` as the default parser guard. Add `--expect-first-line 'AGY_RESEARCH_V1'` only when the chosen model is known to emit the token first.
