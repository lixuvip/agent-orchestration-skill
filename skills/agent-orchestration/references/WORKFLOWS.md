...
3. 除非用户要求严格只读或目标项目禁止修改指令文件，协调者先运行 `scripts/ensure_agy_review_agents_guidance.py --project-root <项目根目录>`，把稳定的 agy/Gemini 命令安全规则写入根目录 `AGENTS.md`；这个步骤必须发生在任何 `agy` 健康检查、模型探测或外部审查命令之前。只有看到 `AGENTS_GUIDANCE_PRESENT` 或 `AGENTS_GUIDANCE_WRITTEN` 才能认为持久规则已就绪；如果写入本应允许但失败，则外部审查降级为 Codex-only 并明确说明原因。
4. 协调者使用 `templates/agy_gemini_review_prompt.template.md` 或中文版本构造只读审查 prompt。
5. `agy` 默认使用普通只读 print + `--sandbox` 模式运行，优先通过 `scripts/run_agy_print.py` 调用；review 或调研场景不要额外加 `--mode` 参数。工具探测只允许 `command -v agy` 和 `agy models`，不要去跑 `command -v gemini`、`gemini --version` 或 `gemini --help`。全项目或仓库审查时要显式传 `--add-dir <项目根目录>`，否则 `agy --print` 可能只看到 scratch workspace；健康检查和结构化审查优先加 `--expect-substring <稳定标记>`，把 0 输出或 narration-only 输出直接打成失败；如果手写命令，`--print` 后面必须紧跟 prompt，不允许在二者之间插入 `--model` 等参数，也不允许 `accept-edits` 或跳过权限。
...
10. 协调者使用 `templates/agy_gemini_review_quality_log.template.md` 或中文版本准备质量日志 JSON，然后运行 `scripts/append_agy_review_quality_log.py --project-root <项目根目录>` 写入 `.codex/agent-orchestration/agy-review-quality.jsonl`；只有看到 `LOG_WRITTEN <path>` 才能声明日志已写入。如果脚本失败、目标项目禁止写入或用户要求只读，则在回复里展示未写入的 JSONL 记录和失败原因。
...
3. 除非用户要求严格只读或目标项目禁止修改指令文件，协调者先运行 `scripts/ensure_agy_review_agents_guidance.py --project-root <项目根目录>`，让稳定的 `agy --print` 命令安全规则持久化到根目录 `AGENTS.md`；这个步骤必须发生在任何 `agy` 健康检查、模型探测或外部调研命令之前。若写入本应允许但失败，则外部调研降级为 Codex-only 并说明原因。
4. 协调者自己做一轮 Codex 调研，必要时补充源码阅读、项目文档和外部一手资料。
5. 协调者使用 `templates/agy_gemini_research_prompt.template.md` 或中文版本构造外部只读调研 prompt，并通过 `scripts/run_agy_print.py` 执行；工具探测只允许 `command -v agy` 和 `agy models`，不要去跑 `command -v gemini`、`gemini --version` 或 `gemini --help`。仓库调研要显式带 `--add-dir <项目根目录>`，结构化输出优先加 `--expect-substring AGY_RESEARCH_V1`。
...
10. 协调者使用 `templates/agy_gemini_research_quality_log.template.md` 或中文版本准备质量日志 JSON，然后运行 `scripts/append_agy_review_quality_log.py --project-root <项目根目录>` 写入 `.codex/agent-orchestration/agy-review-quality.jsonl`；调研场景必须把 `task_type` 设为 `research`。只有看到 `LOG_WRITTEN <path>` 才能声明日志已写入。
