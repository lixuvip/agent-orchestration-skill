...
每次创建新的用户可见子对话前，协调者都要单独评估认知难度；编排模式与思考级别相互独立。综合预期质量、歧义程度、推理深度、影响范围、验证难度和延迟/成本，选择最适合且工具支持的级别。质量、风险覆盖和验证可靠性优先，低档不是默认目标。
...
- 只有相邻的受支持级别对预期质量、风险覆盖和验证可靠性同样适合时，才优先选择较低级别。
- 只有创建工具暴露 `thinking` 时才传值。不需要覆盖时省略并记录 `INHERITED`；工具无法设置时记录 `UNSUPPORTED` 和继承回退，不能谎称已应用。
- 精确级别不可用时，选择最接近且能维持必要风险覆盖的受支持级别；高风险或高歧义任务向上取档而不是向下取档，并记录期望值、实际值和回退原因。
